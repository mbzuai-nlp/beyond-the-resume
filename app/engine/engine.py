...
async def construct_next_message(
	...
	with PreviousAwareJudge(exp=None, use_cache=False) as judge:
		while num_turns < max_turns:
			logging.info(f"Getting judgement for turn {num_turns}")
			judgement = await asyncio.to_thread(
				lambda: judge.judge_next_turn(
					resumes={0: resume_content},
					interviews={0: interview},
					rubrics={0: RUBRIC}
				)[0]
			)
			...
			user_msg = yield interviewer_message, asdict(judgement)
